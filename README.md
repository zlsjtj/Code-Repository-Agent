...
这个项目目前更接近一个本科阶段的练手项目，而不是完整产品。重点放在主流程是否跑得通，以及模型参与代码任务时有哪些地方需要约束。
...
- 把 `benchmarks/smoke_cases.json` 做成可执行脚本，至少能检查回答里是否出现预期引用文件。
- 给前端补一两个端到端冒烟测试，例如导入仓库、触发索引、打开问答面板。
- 改进 `find_symbol`，现在只是轻量匹配，复杂项目里容易漏掉定义。
- 记录一次真实使用截图或短 GIF，让别人不用本地启动也能看懂主要流程。

...
const zhCN: LocaleCopy = {
  workspace: {
    eyebrow: "代码库工作台",
    title: "代码库问答与改动助手",
    subtitle: "在同一个工作台里完成仓库导入、证据驱动问答、改动草案预览和安全校验。",
    backend: "后端状态",
    readyRepos: "可用仓库",
    recentSessions: "最近会话",
    ...
    activeStage: "当前任务",
    tabs: {
      chat: { label: "问答", hint: "围绕当前仓库提问，并查看引用证据。" },
      patch: { label: "改动草案", hint: "先生成差异预览，再决定是否应用。" },
      checks: { label: "检查验证", hint: "运行白名单内的检查命令。" },
    },
    localeLabel: "界面语言",
  },
  repositoryImport: {
    title: "导入仓库",
    description: "支持导入本地仓库，也支持将 GitHub 仓库克隆到受管目录后继续使用。",
    name: "仓库名称（可选）",
    ...
    localPath: "本地仓库路径",
    localPathPlaceholder: "例如：E:\\projects\\demo-repo",
    githubUrl: "GitHub 仓库地址",
    githubUrlPlaceholder: "https://github.com/org/repo",
    defaultBranch: "默认分支",
    defaultBranchPlaceholder: "main",
    help: "本地仓库会直接复用原始路径；GitHub 仓库会先克隆到受管 repos 目录，再注册为工作区。",
    submit: "登记仓库",
    submitting: "提交中...",
    reset: "重置表单",
  },
  repositoryList: {
    title: "仓库列表",
    description: "这里展示已写入 SQLite 的仓库记录，并提供索引和切换上下文的入口。",
  ...
  jobs: {
    title: "最近任务",
    description: "查看最近的后台克隆和索引任务，了解工作区正在处理什么。",
    empty: "还没有后台任务记录。",
    repositoryPrefix: "仓库",
    noMessage: "暂无附加说明。",
    createdLabel: "创建于",
  ...
  chatHistory: {
    title: "最近会话",
    description: "保留当前页面内的问答历史，方便回看和切换。",
  ...
  chat: {
    title: "代码问答",
    description: "选择一个已完成索引的仓库，让 Agent 先检索，再基于证据回答问题。",
  ...
  citations: {
    ...
    empty: "发起一次问答后，这里会显示文件路径、行号、摘录和工具调用轨迹。",
    evidenceCount: "证据片段",
    toolSteps: "工具步骤",
    latency: "响应耗时",
    traceSummary: "调用轨迹",
    toolCalls: "本次回答共调用 {count} 个工具步骤。",
    noCitations: "这次回答没有返回引用，通常意味着 Agent 明确说明证据不足。",
    symbolPrefix: "符号",
  },
  patch: {
    title: "改动草案",
    ...
    missingWorkspace: "当前仓库还没有可用工作区。先确认它已导入本地路径，或已完成克隆。",
    ...
    noDiff: "这次草案没有生成文本差异。通常意味着提示不够具体，或模型判断当前文件无需修改。",
    batchSummary: "批量草案摘要",
    batchModeTitle: "批量预览模式",
    batchModeCopyWithRecommended: (count) =>
      `这批改动已经拿到 ${count} 项推荐检查，可以直接在下方运行。`,
    batchModeCopyDefault: "这批改动已进入推荐检查流程；如果没有更具体的规则，会退回到全部可用检查。",
    selectedCount: "已选文件",
    changedFiles: "变更文件",
    ...
    batchHelp: "选中的文件会作为一次整体提交。后端会先完成整批哈希校验，再统一写入；如有冲突，整批都会被拒绝。",
    combinedDiff: "合并差异预览",
    ...
    noTextualDiff: "这个文件没有生成可应用的文本差异。通常意味着提示不够具体，或当前文件无需修改。",
  },
  checks: {
    title: "代码检查",
    description: "这里只运行自动发现的白名单命令，例如 pytest、typecheck、lint 和 test。",
    selectRepository: "先选择一个仓库，再查看可运行的检查项。",
    loadingProfiles: "正在扫描可运行的检查项...",
    emptyProfiles: "当前仓库还没有发现白名单内的检查项。后端目前只识别 pytest 和常见 npm 脚本。",
    patchApplied: "最近已经把改动写入工作区。现在可以运行检查，确认修改没有破坏现有行为。",
    ...
    runRecommended: "运行推荐检查",
    runDefault: "运行默认检查",
    running: "运行中...",
    runHelp: "当前会按发现顺序串行执行所有白名单检查，并返回标准输出和错误输出摘要。",
    summaryTitle: "检查结果摘要",
    resultCount: "项结果",
    truncated: "输出过长，当前仅显示截断后的摘要。",
  },
  feedback: {
    ...
    answerReady: "问答已完成，可以继续查看引用和工具摘要。",
...
const en: LocaleCopy = {
  ...
  repositoryImport: {
    ...
    localPathPlaceholder: "For example: E:\\projects\\demo-repo",
  ...
  checks: {
    ...
    running: "Running...",
    runHelp: "Checks run serially in discovery order and return summarized stdout and stderr output.",
